...
export default async function QuoteDetailPage({
  ...
  const message = await messageService.getMessageById("QUOTE", resolvedParams.id);

  if (!message) {
    notFound();
  }

  const mailtoLink = `mailto:${message.email}?subject=Re: Request for Quote - PT TSC`;
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-6 bg-gray-50 border-t border-gray-100 flex flex-wrap gap-3 justify-between items-center">
          <DeleteMessageDialog id={message.id} type="QUOTE" />
          <a
            href={mailtoLink}
            target="_blank" 
