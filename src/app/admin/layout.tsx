...
export default async function AdminLayout({
    ...
    // Ambil user info kalau ada token (tidak redirect di sini, biarkan middleware)
    let user: any = null;
    try {
    ...
    // Kalau tidak ada user (halaman login), render children langsung tanpa sidebar
    if (!user) {
        return <>{children}</>;
    }
            ...
            <header className="sticky top-0 z-50 border-b bg-[#071A14] text-white">
                <div className="mx-auto flex h-16 items-center justify-between px-8">
                    <Link href="/admin/dashboard" className="flex items-center gap-2">
                        <span className="text-3xl font-bold">TSC</span>
            ...
            <div className="mx-auto flex">
                ...
                <aside className="w-64 border-r bg-white min-h-[calc(100vh-64px)]">
                    <nav className="space-y-1 p-5">
                        <Link href="/admin/dashboard" className="flex items-center gap-3 rounded-lg px-4 py-3 hover:bg-[#59D66F]/10 hover:text-[#1F6B45]">
                            <LayoutDashboard size={20} /> Dashboard
                        </Link>
                        <Link href="/admin/projects" className="flex items-center gap-3 rounded-lg px-4 py-3 hover:bg-[#59D66F]/10 hover:text-[#1F6B45]">
